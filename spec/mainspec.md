...
+-------------------+--------------------+--------------------+
Core Architectural PrinciplesNative CSS Theming: The component inherits the host color-scheme, uses light-dark() defaults, and exposes semantic --vt-* custom properties for application overrides.Resolution-Driven Snapping: Drag-and-drop operations (moving and edge-resizing) snap to grid intervals matching the configured daily resolution $R$ by default. Fine-grained, sub-resolution timestamps are preserved and can be supplied by host applications.Decoupled Scale & Resolution: Visual vertical space is governed by an independent dayHeight parameter ($H_{day}$), allowing zoom and grid subdivision ($R$) to be modified independently without distorting overall date geometry.Timezone Context & Dynamic Inheritance: Events store explicit timezone designations. Newly created events inherit the timezone of the chronologically closest preceding event; host applications control timezone behavior when editing.2. Component API & TypeScript Interfacesimport { ReactNode } from 'react';
